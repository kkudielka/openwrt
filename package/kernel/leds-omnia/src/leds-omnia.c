...
omnia_dt_init(struct i2c_client *client)
{
	...
	leds = devm_kcalloc(&client->dev,
			MAX_LEDS, sizeof(struct led_info), GFP_KERNEL);
	if (!leds)
...
static struct attribute *omnia_led_attrs[] = {
	&dev_attr_autonomous.attr,
	&dev_attr_color.attr,
	NULL
};
ATTRIBUTE_GROUPS(omnia_led);
					...
					const struct i2c_device_id *id)
{
	...
	if (!chip)
		return -ENOMEM;
	leds = devm_kcalloc(&client->dev, MAX_LEDS, sizeof(*leds),
				GFP_KERNEL);
	if (!leds)
		return -ENOMEM;
	...
	for (i = 0; i < MAX_LEDS; i++) {
		...
		leds[i].led_cdev.groups = omnia_led_groups;
